  ...
  async getMessage(accountId: string, messageId: string): Promise<NylasMessage> {
    try {
      ...
      return {
        ...
        from: messageData.from?.map(f => ({ name: f.name, email: f.email })) || [],
        to: messageData.to?.map(t => ({ name: t.name, email: t.email })) || [],
        cc: [],
        bcc: [],
        reply_to: messageData.replyTo?.map(r => ({ name: r.name, email: r.email })),
        date: messageData.date || Date.now() / 1000,
        unread: messageData.unread || false,
        starred: messageData.starred || false,
        snippet: messageData.snippet || '',
        body: messageData.body || '',
        files: messageData.attachments?.map(att => ({
          id: att.id || '',
          filename: att.filename || '',
          content_type: att.contentType || '',
          size: att.size || 0,
        })) || [],
        labels: messageData.folders?.map(f => ({ id: f, name: f })) || [],
        folder: { id: 'inbox', name: 'INBOX' },
      };
    } catch (error) {
