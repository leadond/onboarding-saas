-- OnboardKit Supabase Database Setup (Clean Version)
...
-- Add unique constraints that couldn't be added inline
DO $$ 
BEGIN
  -- Kit owner + slug unique constraint
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'kits_owner_slug_unique'
  ) THEN
...
-- Kits: Users can manage their own kits, published kits are viewable
...
-- Client Progress: Kit owners can view progress
...
-- User Integrations: Users manage their own integrations
