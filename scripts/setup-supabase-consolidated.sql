-- OnboardKit Supabase Database Setup with Data Access Requests
-- This script creates all necessary tables with proper relationships and security policies
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE public.organization_members ADD COLUMN user_id UUID;
  END IF;
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public'
    AND table_name = 'kit_steps'
    AND column_name = 'order_index'
  ) THEN
    ALTER TABLE public.kit_steps ADD COLUMN order_index INTEGER;
  END IF;
  ...
  IF NOT EXISTS (
    ...
    WHERE table_schema = 'public'
    AND table_name = 'kit_steps'
    AND column_name = 'kit_id'
  ) THEN
    ALTER TABLE public.kit_steps ADD COLUMN kit_id UUID;
  END IF;
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public'
    AND table_name = 'client_progress'
    AND column_name = 'kit_id'
  ) THEN
    ALTER TABLE public.client_progress ADD COLUMN kit_id UUID;
  END IF;
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE public.client_progress ADD COLUMN client_email TEXT;
  END IF;
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE public.user_integrations ADD COLUMN provider TEXT;
  END IF;
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE public.user_integrations ADD COLUMN user_id UUID;
  END IF;
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE public.kits ADD COLUMN created_by UUID;
  END IF;
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE public.kits ADD COLUMN slug TEXT;
  END IF;
...
DROP TRIGGER IF EXISTS handle_updated_at_data_access_requests ON public.data_access_requests;
CREATE TRIGGER handle_updated_at_data_access_requests BEFORE UPDATE ON public.data_access_requests
  FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();
...
-- =============================================
-- STEP 6: CREATE DATA ACCESS REQUEST FUNCTIONS
-- =============================================
